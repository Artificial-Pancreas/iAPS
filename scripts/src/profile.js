...
module.exports = (iaps_input) => {
  ...
  if (carbratio_input) {
    var errors = [ ];
    if (!(carbratio_input.schedule && carbratio_input.schedule[0].offset !== undefined && carbratio_input.schedule[0].ratio)) {
      errors.push("Carb ratio data should have an array called schedule with a offset and ratio fields.");
    }
  ...
  var profile = oref0_profile(logs, inputs, defaults, clock);
