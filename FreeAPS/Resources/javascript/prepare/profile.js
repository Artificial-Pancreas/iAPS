...
function generate(
  ...
  carbratio_input = false,
  temptargets_input = false,
  model_input = false,
  autotune_input = false,
  freeaps_data,
  dynamicVariables_input,
  settings_input
) {
  ...
  if (preferences_input) {
    ...
    if (preferences.curve === 'rapid-acting') {
      if (preferences.useCustomPeakTime) {
        preferences.insulinPeakTime = Math.max(50, Math.min(preferences.insulinPeakTime, 120));
      } else {
        preferences.insulinPeakTime = 75;
      }
    } else if (preferences.curve === 'ultra-rapid') {
      if (preferences.useCustomPeakTime) {
        preferences.insulinPeakTime = Math.max(35, Math.min(preferences.insulinPeakTime, 100));
      } else {
  ...
  if (autotune_data) {
    if (autotune_data.basalprofile) {
      inputs.basals = autotune_data.basalprofile;
    }
    if (!freeaps.onlyAutotuneBasals) {
      if (autotune_data.isfProfile) {
        inputs.isf = autotune_data.isfProfile;
      }
      if (autotune_data.carb_ratio) {
        inputs.carbratio.schedule[0].ratio = autotune_data.carb_ratio;
      }
    }
  }

  // merge oref0 defaults with iAPS ones
  const defaults = Object.assign({}, freeaps_profile.defaults(), {
