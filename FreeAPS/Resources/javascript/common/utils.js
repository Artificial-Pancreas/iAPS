...
module.exports.floor = (value, digits) => {
  if (! digits) { digits = 0; }
  const scale = Math.pow(10, digits);
...
// TODO: autoisf has its own copy of exercising but it is different
...
module.exports.disableSMBs = (dynamicVariables, now) => {
  if (dynamicVariables.smbIsOff) {
    ...
    const start = dynamicVariables.start;
    const end = dynamicVariables.end;
