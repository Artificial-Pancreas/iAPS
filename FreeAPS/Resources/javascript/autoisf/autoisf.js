...
function interpolate(xdata, profile, type) {
  // interpolate ISF behaviour based on polygons defining nonlinear functions defined by value pairs
...
function aisf_ratio(profile, glucose_status, currentTime, autosens_data, normalTarget, dynamicVariables) {
  ...
  console.log(
    'AutoISF bg_acceleration: ' +
      round(bg_acce, 2) +
      ', PF-minutes: ' +
      parabola_fit_minutes +
      ', PF-corr: ' +
      round(parabola_fit_correlation, 4) +
      ', PF-nextDelta: ' +
      convert_bg(parabola_fit_next_delta, profile) +
      ', PF-lastDelta: ' +
      convert_bg(parabola_fit_last_delta, profile) +
      ', regular Delta: ' +
      convert_bg(glucose_status.delta, profile)
  );
  ...
  if (!profile.iaps.enableBGacceleration) {
    ...
    if (parabola_fit_a2 !== 0 && parabola_fit_correlation >= 0.9) {
      ...
      if (minmax_delta > 0 && bg_acce < 0) {
        ...
        if (minmax_delta <= 30 && minmax_value < target_bg) {
          // start braking
    ...
    if (parabola_fit_correlation < 0.9) {
      ...
      if (acce_weight === 1 && glucose_status.glucose < target_bg) {
        // below target acce goes towards target
        if (bg_acce > 0) {
          if (bg_acce > 1) {
      ...
      } else if (acce_weight === 1) {
        // above target acce goes away from target
        if (bg_acce < 0) {
          acce_weight = profile.iaps.bgBrakeISFweight;
        } else if (bg_acce > 0) {
          acce_weight = profile.iaps.bgAccelISFweight;
  ...
  } else if (avg05 <= target_bg * 1.05) {
    // Don't use dura below target and don't treat glucose near target as a plateaued glucose (fix)
...
function determine_varSMBratio(profile, bg, dynamicVariables) {
  ...
  if (smb_delivery_ratio_bg_range === 0) {
    // deactivated in Auto ISF setting
    return fix_SMB;
  }
  if (bg <= target_bg) {
...
function iob_max(iob, dynamicVariables, profile) {
  ...
  if (currentIOB >= smbIOB) {
    ...
    const smbIOBRemaining = smbIOB * 1.3 - currentIOB;
