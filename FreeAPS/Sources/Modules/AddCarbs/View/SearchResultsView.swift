...
struct SearchResultsView: View {
    ...
    private func saveMealTotalsAsFoodItem() {
        ...
        let micronutrients: [MicronutrientValue] = mealMicros.map { nutrient, amount in
...
private struct FoodItemGroupListSection: View {
    ...
    private func saveSectionAsFoodItem() {
        ...
        let micronutrients: [MicronutrientValue] = sectionMicros.map { nutrient, amount in
            let amountPer100: Decimal =
                totalServingSize > 0
                    ? amount / totalServingSize * 100
                    : amount
...
enum NutritionAggregator {
    static func micronutrients(from items: [FoodItemDetailed]) -> [MicroNutrient: Decimal] {
