...
extension RecoveryTokenAccountView {
    @MainActor final class Model: ObservableObject {
        ...
        private let profileAndSettingsUploadManager: ProfileAndSettingsUploadManager

        init(resolver: Resolver) {
            self.resolver = resolver
            database = resolver.resolve(Database.self)!
            profileAndSettingsUploadManager = resolver.resolve(ProfileAndSettingsUploadManager.self)!
            token = resolver.resolve(UserToken.self)?.getIdentifier() ?? ""
        }
        ...
        func onAppear() {
            ...
            nudged = true
            Task {
                await profileAndSettingsUploadManager.uploadProfileAndSettings(force: true)
            }
        }
        ...
        func link() {
            ...
            Task {
                do {
                    try await database.linkAccount(
                    ...
                    guard let error = error as? AccountLinkError else {
        ...
        func register() {
            Task {
                ...
                do {
                    ...
                    self.working = false
                    self.messageIsError = true
                    guard let error = error as? AccountRegisterError else {
                        self.message = error.localizedDescription
                        return
                    }
                    switch error {
