...
extension PreferencesEditor {
    final class StateModel: BaseStateModel<Provider>, PreferencesSettable {
        ...
        override func subscribe() async {
            ...
            subscribeSetting(\.maxCarbs, on: $maxCarbs) { self.maxCarbs = $0 }
            subscribeSetting(\.units, on: $unitsIndex.map { $0 == 0 ? GlucoseUnits.mgdL : .mmolL }) {
                self.unitsIndex = $0 == .mgdL ? 0 : 1
            } didSet: { [weak self] _ in
                Task {
                    await self?.saveTargetsAndISF()
                }
            }
            let mainFields = [
                Field(
        ...
        private func savePreferences(_ preferences: Preferences) async {
            var prefs = preferences
            prefs.timestamp = Date()
            await settingsManager.updatePreferences(prefs)
        }

        private func saveTargetsAndISF() async {
            await saveTargets()
            await saveISF()
        }
