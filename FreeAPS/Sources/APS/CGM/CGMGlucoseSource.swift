...
final class CGMGlucoseSource: GlucoseSource {
    private let processQueue = DispatchQueue(label: "CGMGlucoseSource.processQueue")
...
//    func setCGMManagerFromDescriptor(_ descriptor: CGMManagerDescriptor, deviceManager: DeviceDataManager) {
//        // Get CGM manager type from plugin system
//        if let cgmManagerType = deviceManager.cgmManagerTypeByIdentifier(descriptor.identifier) {
//            // Create instance - you'll need to implement this in DeviceDataManager
//            if let cgmManager = deviceManager.createCGMManager(type: cgmManagerType) {
//                setCGMManager(cgmManager: cgmManager)
//                cgmType = CGMType.from(pluginIdentifier: descriptor.identifier) ?? .nightscout
//            }
//        }
//    }

    func setCGMManager(cgmManager: CGMManagerUI) {
        if let current = self.cgmManager {
            current.cgmManagerDelegate = nil
            current.delegateQueue = nil
        }
...
extension CGMGlucoseSource: CGMManagerDelegate {
